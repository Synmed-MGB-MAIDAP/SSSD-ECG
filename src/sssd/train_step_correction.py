    ...
    while n_iter < n_iters:
        ...
        for i, (audio, label) in enumerate(trainloader):
            ...
            if trainset_config['loss_fn'] == 'mel_loss':
                ...
                # print ("reconstructed_x_signal shape:", reconstructed_x_signal.shape, "orig_x_signal shape:", orig_x_signal.shape)
                wandb.log({'step_train_loss': loss.item()}, step=n_iter)
                wandb.log({'step_mel_loss': mel.item()}, step=n_iter)
                wandb.log({'step_mse_loss': mse.item()}, step=n_iter)
            else:
            ...
            if debug and n_iter>0 and n_iter % (iters_per_logging*10) == 0:                
                ...
                print ("num_samples for viz:", num_samples )
            ...
            # =========== Closing of Step/n_iter level code ==================
            n_iter += 1
            pbar.update(1)
...
# nohup python src/sssd/train_step_correction.py -c /home/nutansahoo/MGB-MAIDAP/models/SSSD-ECG/src/sssd/config/SSSD_ECG_mel_loss.json > train_19_11_25.log 2>&1 &
# 1939914